...
Conclusao: O senso comum leva a crer que pelo fato de Sense8 ser a serie mais bem classificada segundo o IMDB (Checkpoint 1) entao, certamente, ela seria a serie mais votada(altos votos levam a boa classificacao). Isto significa que a a classificacao de um episodio segundo o IMDB leva em consideracao algo alem do numero de votos do episodio ja que a serie mais bem votada foi Black Mirror e nao Sense8.

Nova pergunta: Com o intuito de certificar-se se existe ou não uma relação entre a classificacao do episodio segundo o IMDB (UserRating) e o numero de votos (UserVotes), qual o coeficiente de corelacao linear entre essas duas variaveis?
```{r}
...
dados %>%  filter(series_name == "Sense8") %>% 
  ...
  labs(title = "Distribuicao da classificacao ao longo das temporadas", x="Episodio da temporada", y="Classificacao do usuario")
```
O episodio de Black Mirror mais bem avaliado foi o episodios 4 da segunda temporada e o menos avaliado foi o episodio 5 da terceira temporada como podemos ver no grafico abaixo. Quanto a diferenca pode-se dizer que e razoavel, o episodio 4 teve avaliacao superior a 9.0 enquanto o episodio 5 teve quase 8.0(diferenca de pelo menos 1 ponto). 
```{r}
dados %>%  filter(series_name == "Black Mirror") %>% 
  mutate(season = as.character(season)) %>% 
  ggplot(aes(x = season_ep, y = UserRating, color = season)) + 
...
Nova pergunta: Para as series que possuem mais de uma temporada, a avaliacao do primeiro episodio de uma nova temporada mantem ou faz jus a avaliacao do ultimo episodio da temporada anterior? O que quero saber e: a avaliacao do episodios da nova temporada e uma continuidade da avaliacao do ultimo episodio da temporada anterior?
...
```
Olhando para o grafico acima fica facil observar se a avaliacao dos episodios da nova temporada e uma continuidade da avaliacao dos episodios da temporada anterior. Vamos comecar por Black Mirror: a primeira temporada tem apenas 3 episodios, desta forma, o episodio 4 marca o inicio da segunda temporada com queda da avaliacao do episodio em razao de decimos. A transicao da segunda temporada para a terceira(episodios 7-8) podemos observar novamente uma queda da avaliacao do episodio. Em virtude das quedas registradas estarem em razao de decimos entao isso nao deve ser algo preocupante, porém, deve ser algo motivador para que os produtores e diretores procurem aumentar/manter a avaliacao dos episodios da serie.

Sense8 possui apenas duas temporadas: a primeira contendo 12 episodios e a segunda contendo 11 episodios. Observando a transicao dos episodios 12-13 podemos observar, igualmente em Black Mirror, uma queda da avaliacao. Assim como as quedas de avalicao de Black Mirror, a queda em Sense8 e em razao de decimos.

Respondendo a pergunta: a avaliacao dos episodios das novas temporadas pode ser considerado uma continuidade da avaliacao dos episodios da temporada anterior.

...
```
Podemos perceber que segundo a classificacao de usuarios IMDB Sense8 tem apenas um valor estranho(~7.5). Existe variacao e dentre os valores frequentes tem a classificacao 9 como podemos ver no histograma abaixo.
```{r}
dados %>%
  filter(series_name == "Sense8") %>%
...
dados %>%
  ...
  labs(title = "Distribuicao da classificacao do usuario", x="Nome da serie", y="Classificacao do usuario")
```
Podemos perceber que segundo a classificacao de usuarios IMDB Black Mirror tem apenas um valor estranho(~7.0). Existe variacao e dentre os valores frequentes temos as classificacoes de 8.0 e 8.5 como podemos ver no histograma abaixo.
...
Quanto a correlacao entre o episodio da temporada e sua classificacao podemos perceber a existencia de uma correlacao linear, que tende a ser forte, entre essas duas variaveis apenas na primeira temporada.
```{r}
dados %>%  filter(series_name == "Black Mirror") %>% 
    group_by(season) %>% 
    summarise(correlacao_linear = cor(season_ep, UserRating, 
...
dados %>%
  ...
  labs(title = "Distribuicao da classificacao do usuario", x="Nome da serie", y="Classificacao do usuario")
```
Podemos perceber que segundo a classificacao de usuarios IMDB 13 Reasons Why nao possui valores estranhos. Existe pouca variacao se comparado as séries Sense8 e Black Mirror e dentre os valores frequentes temos as classificacoes de 8.5 como podemos ver no histograma abaixo.
